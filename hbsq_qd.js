...
let hbspqdbody = $.getdata('hbspqdbody');
    ...
    if (typeof $request !== "undefined") {
        ...
        for (let i = 0; i < hbsphdArr.length; i++) {
            if (hbsphdArr[i]) {
                ...
                hbsphd = hbsphdArr[i];
                hbspqdbody = hbspqdbodyArr[i];
                $.index = i + 1;
                console.log(`\n【 红包视频 账号${$.index} 】`)
                //console.log(hbspqdbody)             
...
function hbspqdck() {
    if ($request.url.indexOf("withdraw") > -1) {
        ...
        const hbspurl = $request.url
        if (hbspurl) $.setdata(hbspurl, `hbspurl${status}`)
        $.log(hbspurl)
        ...
        const hbspqdbody = $request.body
        if (hbspqdbody) $.setdata(hbspqdbody, `hbspqdbody${status}`)
        $.log(hbspqdbody)
...
//签到 https://hbapi.qudianyue.com/video?&t=${times}&m=getGold 
            ...
            try {
                ...
                if (data.result == 1) {
                   console.log(`今天已签到`)
                    ...
                    console.log(`今天你签到获得金币:`+data.result)
